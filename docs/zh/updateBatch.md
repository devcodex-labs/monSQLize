...
每个候选文档在提交写入时会再次校验原始业务过滤条件；不再匹配的文档计为 skipped。批次发生致命错误后不再领取新批次，已开始的批次完成后才返回错误。严格 Model 更新中，`conflictCount` 表示版本不匹配；业务条件变化或并发删除计为 skipped。
...
- [使用示例](https://github.com/devcodex-labs/monSQLize/blob/main/examples/docs/batch-operations.ts)
